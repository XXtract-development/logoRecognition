#!/usr/bin/env python3
"""
Story 12.2 spike — class-agnostic region proposer (option B: classical CV).

Finds "logo/mark-like" candidate regions on packaging artwork WITHOUT any per-class
template — so its cost is INDEPENDENT of the number of keurmerk classes. Downstream,
each proposed region is embedded once and classified via the existing pgvector
reference index (cost ~ regions x ANN), decoupling detection cost from #classes.

This POC only does the PROPOSAL stage and reports region count + latency. It combines
MSER (stable blobs, good for logos/text) with contour proposals (adaptive threshold),
filters by relative size / aspect / solidity, and de-duplicates via IoU-NMS.

Usage (in ML container, has cv2):  python3 spike_region_proposer.py <image.png> [--viz out.png]
"""
...
def _nms(boxes: list[tuple[int, int, int, int]]) -> list[tuple[int, int, int, int]]:
...
def propose_regions(image_bgr: np.ndarray) -> tuple[list[tuple[int, int, int, int]], dict]:
...
def main() -> int:
    ...
    if img is None:
        ...
        return 2
    boxes, stats = propose_regions(img)
    print(stats)
    if args.viz:
        vis = img.copy()
