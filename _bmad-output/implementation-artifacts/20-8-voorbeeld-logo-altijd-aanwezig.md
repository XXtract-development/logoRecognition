...
- 2026-07-21: LIVE + geverifieerd. Code was al gedeployed (171fdfc, fallback aanwezig in de gedraaide app-build). Voorbeelden geüpload naar ACC `reference-examples/<code>.png` en OPGESCHOOND: van de gids-extractie zijn alleen de app-bekende codes behouden (frontend-universe ∪ ACC-codes = 952). Eindstand bucket: **881 voorbeelden, 0 junk** (927 verwijderd = 904 macOS-AppleDouble `._`-bestanden die per ongeluk meeliftten in de tar + 23 niet-opvraagbare regulator-/AISE-GDS-namen; AISE_x heeft al echte refs). Endpoint-verificatie op ACC: BDIH_LOGO/ICADA -> 200 `guide-example`, GREEN_DOT -> 200 `reference`, SOCIETY_PLASTICS_INDUSTRY -> 404 -> frontend-placeholder (vector-only in gids). Feature volledig werkend.