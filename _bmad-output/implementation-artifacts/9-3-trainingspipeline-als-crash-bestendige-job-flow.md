...
2. **Vier step-workers** (`apps/api/src/services/pipeline/workers.ts`):
   ...
   - `registerTrainingFlowWorker()` met **Worker-concurrency=1** (AC3 — BullMQ-concurrency is een Worker-optie, niet job-optie). Geregistreerd bij API-opstart in `main.ts`, naast de trigger-cron-worker.
...
**Tests**: api-vitest 236 passed / 2 skipped (was 225/2; +11 worker-tests). tsc schoon (na `prisma generate`). pytest synthesis/artwork-pad: 24 passed (21 artwork + 3 nieuw endpoint). De `test_holdout_trainer.py`-fouten zijn pre-existing (conftest mockt `app.services.trainer` als MagicMock; die suite vereist de echte trainer + torch, niet draaibaar in de kale venv) — niet door deze story veroorzaakt.
